...
html{scroll-behavior:smooth;font-size:clamp(1rem, 0.5vw + 0.5rem, 1.2rem);} 
body{background:var(--bg);color:var(--ink);font-family:'DM Sans',system-ui,sans-serif;overflow-x:hidden;}
/* Hide mobile menu button by default; media query will show it */
...
.aura-orb-1{width:40vmin;height:40vmin;top:-10vmin;left:-10vmin;}
.aura-orb-2{width:30vmin;height:30vmin;top:-5vmin;right:-7.5vmin;}
.aura-orb-3{width:35vmin;height:35vmin;bottom:-10vmin;right:-5vmin;}
.aura-orb-4{width:25vmin;height:25vmin;bottom:10%;left:-5vmin;}
...
.nav-link::after{content:'';position:absolute;left:0;bottom:-0.1875rem;width:0;height:0.09375rem;
...
.story-card::before{content:'';position:absolute;top:0;left:0;width:0.25rem;height:100%;
...
.svc-card::before{content:'';position:absolute;width:17.5rem;height:17.5rem;right:-3.75rem;top:-5rem;
...
.phil-book-card::before{content:'';position:absolute;left:0;top:0;bottom:0;width:0.1875rem;
...
@media (max-width: 768px){
  nav{height:4rem !important;padding:0 1rem !important;}
  ...
  /* Mobile-specific card stacking and size adjustments */
  .course-level-card{display:flex !important;flex-direction:column !important;align-items:flex-start !important;padding:0.75rem 0.9rem !important;margin:0 !important}
  .course-level-card .course-level-tag{align-self:flex-end !important;margin-top:0.6rem !important}
  .course-level-card > div{width:100%}
  .svc-card .inline-grid, .fw-card .inline-grid{grid-template-columns:1fr !important}

  /* Reduce large section paddings for small screens */
  header, .hero, .hero-grid{padding-top:3.5rem !important}
  section#services, section#student-reading{padding:3rem 0 !important}

  /* Ensure nav button is visible and accessible */
  nav > button{width:44px;height:44px;display:flex!important;align-items:center;justify-content:center;border-radius:8px}

  /* Slightly smaller base font on mobile for denser layout */
  html{font-size:0.95rem}

  /* Slightly smaller primary buttons and tighter spacing */
  .btn-primary{padding:0.6rem 1.4rem !important;font-size:0.72rem !important}
  .course-level-card{padding:0.6rem !important}

  /* Mobile menu overlay full-screen */
  .mobile-overlay{position:fixed !important;top:0;left:0;right:0;height:100vh;overflow:auto;z-index:1200;padding:1.25rem;background:rgba(247,245,254,0.98);backdrop-filter:blur(12px)}
...
function Navbar() {
  ...
  // Accessibility: focus first link when mobile menu opens, and close on Escape
  useEffect(() => {
    function onKey(e) {
    ...
    if (mobileOpen) {
      ...
      setTimeout(() => {
        firstMobileLinkRef.current && firstMobileLinkRef.current.focus();
      }, 50);
    } else {
    ...
    <nav style={{
      ...
      >
        <svg width="22" height="16" viewBox="0 0 22 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden>
          <rect x="0" y="1" width="22" height="2" rx="1" fill={scrolled ? 'var(--ink)' : '#fff'} />
          <rect x="0" y="7" width="22" height="2" rx="1" fill={scrolled ? 'var(--ink)' : '#fff'} />
          <rect x="0" y="13" width="22" height="2" rx="1" fill={scrolled ? 'var(--ink)' : '#fff'} />
        </svg>
      </button>

      <div style={{display:"flex",flexDirection:"column",gap:1}}>
        <span className="font-display" style={{fontSize:"clamp(1.4rem, 3vw, 2rem)",fontWeight:700,color:"var(--ink)",lineHeight:1}}>M L Suriya</span>
        <span style={{fontSize:"clamp(0.65rem, 1vw, 0.9rem)",fontWeight:600,letterSpacing:"0.26em",textTransform:"uppercase",color:"var(--p500)"}}>Peaceful Profit Monk</span>
      </div>

      <ul className="nav-links" style={{display:"flex",gap:"3rem",listStyle:"none"}}>
        {links.map((l,i)=>(
          <li key={l}><a href={hrefs[i]} className="nav-link" style={{fontSize:"clamp(0.8rem, 1.2vw, 1.1rem)",fontWeight:600,letterSpacing:"0.08em",color:"var(--ink2)",textDecoration:"none",position:"relative"}}>{l}</a></li>
        ))}
      </ul>

      <a href="tel:+919427611171" className="call-now" style={{fontSize:"clamp(0.85rem, 1.3vw, 1.1rem)",fontWeight:600,letterSpacing:"0.12em",textTransform:"uppercase",color:"#fff",background:"linear-gradient(135deg,var(--p700),var(--p500))",padding:"0.68rem 1.7rem",borderRadius:"100px",textDecoration:"none",boxShadow:"0 4px 20px rgba(107,53,200,0.38)",transition:"all 0.3s"}}>Call Now</a>
    </nav>
      ...
      <div className="mobile-overlay" role="dialog" aria-modal="true">
        <div style={{display: "flex", flexDirection: "column", gap: "1rem"}}>
          <div style={{display:'flex',justifyContent:'flex-end'}}>
            <button aria-label="Close menu" onClick={()=>setMobileOpen(false)} style={{background:'transparent',border:'none',padding:8,fontSize:18}}>✕</button>
          </div>
          <div>
            {links.map((l,i)=>(
              <a key={l} href={hrefs[i]} ref={i===0?firstMobileLinkRef:null} style={{
                display: "block",
                padding: "0.75rem 1rem",
                fontSize: "1.1rem",
                fontWeight: 600,
                letterSpacing: "0.08em",
                color: scrolled ? "var(--ink)" : "#1f0f3f",
                textDecoration: "none",
                borderRadius: "8px",
                marginBottom: "0.25rem",
                transition: "background 0.2s",
                "&:hover": {
                  background: scrolled ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.05)"
                }
              }}>{l}</a>
            ))}
          </div>
          <a href="tel:+919427611171" style={{
            display: "block",
            ...
            borderRadius: "100px",
            textDecoration: "none",
            boxShadow: "0 4px 20px rgba(107,53,200,0.38)",
            transition: "all 0.3s",
          }}>Call Now</a>
        </div>
      </div>
...
function Hero() {
  return (
    <section style={{minHeight:"100vh",display:"flex",alignItems:"flex-start",paddingTop: "4.875rem",overflow:"hidden",position:"relative",isolation:"isolate"}}>
      <div style={{width:"100%"}}>
        <div style={{
          display:"grid",gridTemplateColumns:"1.06fr 0.94fr",alignItems:"stretch",position:"relative",zIndex:1,minHeight: "40rem",
            background:"linear-gradient(112deg,rgba(255,255,255,0.96) 0%,rgba(251,247,255,0.93) 56%,rgba(77,35,145,0.92) 100%)",
          borderBottom:"1px solid rgba(255,255,255,0.75)",boxShadow:"0 1.75rem 4.75rem rgba(52,28,96,0.25)",
        }}>
          {/* Left */}
          <div style={{display:"flex",flexDirection:"column",justifyContent:"flex-start",padding:"3.4rem 3.1rem 3rem",position:"relative",zIndex:2,background:"linear-gradient(140deg,rgba(255,255,255,0.97),rgba(249,244,255,0.95))"}}>
            {/* Om symbol */}
            <div style={{position:"relative",width: "5.125rem",marginBottom:"1.2rem",display:"flex",flexDirection:"column",alignItems:"flex-start",gap:"0.5rem"}}>
              <div style={{position:"absolute",width: "3.5rem",height: "3.5rem",top: "2rem",left: "2rem",transform:"translate(-50%,-50%)",borderRadius:"50%",border:"1px solid rgba(240,120,64,0.35)"}} className="ring-1"/>
              <div style={{position:"absolute",width: "4.875rem",height: "4.875rem",top: "2rem",left: "2rem",transform:"translate(-50%,-50%)",borderRadius:"50%",border:"1px dashed rgba(139,92,246,0.22)"}} className="ring-2"/>
              <div style={{position:"absolute",width: "6.25rem",height: "6.25rem",top: "2rem",left: "2rem",transform:"translate(-50%,-50%)",borderRadius:"50%",border:"1px dotted rgba(232,121,249,0.14)"}} className="ring-3"/>
              <div className="om-animate" style={{width: "4rem",height: "4rem",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"2.4rem",lineHeight:1,background:"linear-gradient(135deg,var(--gold),var(--coral),var(--rose))",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text",position:"relative",zIndex:2}}>ॐ</div>
            </div>
            <span style={{fontSize:"clamp(0.55rem, 0.9vw, 0.75rem)",fontWeight:600,letterSpacing:"0.3em",textTransform:"uppercase",color:"var(--p600)",marginBottom:"1rem"}}>Peaceful Profit Monk · ध्यान धन साधक</span>
            ...
            <p style={{fontSize:"clamp(0.9rem, 1.5vw, 1.2rem)",lineHeight:1.58,color:"var(--ink2)",fontWeight:300,maxWidth:620,marginBottom:"2.4rem"}}>For over <strong style={{color:"var(--p600)",fontWeight:600}}>30 years</strong>, guiding 300+ organisations to double their profits — through peace, dharma &amp; the timeless science of meditation. Govt. of India certified ZED Master Trainer.</p>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"0.75rem",marginBottom:"2.2rem"}}>
              {[["30+","Years of Mastery"],["300+","Organisations"],["2×","Profit Growth"]].map(([n,l])=>(
                <div key={l} style={{background:"rgba(255,255,255,0.52)",backdropFilter:"blur(18px)",border:"1px solid rgba(255,255,255,0.84)",borderRadius:14,boxShadow:"0 4px 20px rgba(107,53,200,0.08)",padding:"1.1rem 1.4rem",textAlign:"center"}}>
                  ...
                  <span style={{fontSize:"clamp(0.5rem, 1vw, 0.8rem)",fontWeight:500,letterSpacing:"0.08em",textTransform:"uppercase",color:"var(--muted)",display:"block",marginTop:"0.3rem"}}>{l}</span>
                </div>
              ))}
            </div>
          ...
          <div style={{position:"relative",overflow:"hidden",minHeight:"100%",display:"flex",alignItems:"flex-end"}}>
            ...
            <div style={{position:"relative",left:"5.5%",right:"5.5%",bottom:"1.1rem",width:"89%",display:"flex",flexDirection:"column",gap:"0.8rem",zIndex:3,marginBottom:"1.1rem"}}>
              <div className="glass" style={{padding:"1.45rem 1.2rem",background:"rgba(67,38,120,0.54)",border:"1px solid rgba(255,255,255,0.32)",boxShadow:"0 10px 38px rgba(10,4,22,0.35)",borderRadius:20}}>
                <span style={{fontSize:"clamp(0.5rem, 0.9vw, 0.7rem)",fontWeight:600,letterSpacing:"0.28em",textTransform:"uppercase",color:"var(--p500)",display:"block",marginBottom:"0.9rem"}}>Credentials &amp; Expertise</span>
                  ...
                  {[
                    ...
                    return (
                      <div key={i} style={{display:"flex",alignItems:"flex-start",gap:"0.9rem",padding:"0.78rem 0.85rem",background:"rgba(255,255,255,0.55)",borderRadius:12,border:"1px solid rgba(255,255,255,0.9)"}}>
                        <div style={{width:0.5rem,height:0.5rem,borderRadius:"50%",flexShrink:0,marginTop:0.25rem,background:dotColors[color],boxShadow:`0 0 0.5rem ${dotShadows[color]}`}}/>
                        <div>
                          <span style={{fontSize:"clamp(0.6rem, 1vw, 0.9rem)",fontWeight:600,color:"var(--ink)",display:"block",lineHeight:1.3}}>{title}</span>
                          <span style={{fontSize:"clamp(0.5rem, 0.9vw, 0.8rem)",color:"var(--muted)",display:"block",marginTop:"0.1rem"}}>{sub}</span>
                        </div>
                      </div>
                    );
                  })}
              ...
              <a href="https://www.youtube.com/@peacefulprofitmonk" target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",gap:"0.85rem",padding:"0.85rem 1.1rem 0.85rem 0.9rem",background:"rgba(255,255,255,0.50)",backdropFilter:"blur(18px)",border:"1px solid rgba(255,255,255,0.80)",borderRadius:16,textDecoration:"none",color:"inherit",boxShadow:"0 4px 24px rgba(107,53,200,0.09)"}}>
                ...
                <div>
                  <span style={{fontSize:"clamp(0.6rem, 1vw, 0.9rem)",fontWeight:600,color:"var(--ink)",display:"block",lineHeight:1.3}}>Watch Daily Dhyan &amp; Swadhyaya Sessions</span>
                  <span style={{fontSize:"clamp(0.5rem, 0.9vw, 0.7rem)",color:"var(--muted)",display:"block",marginTop:"0.08rem"}}>@PeacefulProfitMonk · Free · Open to All</span>
                </div>
                <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:"0.3rem",fontSize:"clamp(0.45rem, 0.8vw, 0.7rem)",fontWeight:700,letterSpacing:"0.14em",textTransform:"uppercase",color:"rgba(255,80,80,0.9)",border:"1px solid rgba(255,80,80,0.25)",padding:"0.22rem 0.55rem",borderRadius:100,background:"rgba(255,68,68,0.07)"}}>
      ...
      <div className="marquee-anim" style={{display:"inline-flex",whiteSpace:"nowrap"}}>
        {doubled.map((item,i)=>(
          <span key={i} style={{display:"inline-flex",alignItems:"center"}}>
            <span style={{fontSize:"clamp(0.6rem, 1vw, 0.9rem)",fontWeight:500,letterSpacing:"0.2em",textTransform:"uppercase",color:"var(--ink2)",padding:"0 2.5rem"}}>{item}</span>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1.5fr",gap:"5rem",alignItems:"start"}}>
          ...
          <div className="reveal" style={{position:"sticky",top:90}}>
            <div style={{position:"relative",width:"100%",marginBottom:"2.2rem"}}>
              ...
              <div className="badge-float-1" style={{position:"absolute",zIndex:3,top:"12%",right:-18,background:"rgba(255,255,255,0.90)",backdropFilter:"blur(16px)",border:"1px solid rgba(255,255,255,0.95)",borderRadius:100,padding:"0.46rem 0.9rem",fontSize:"clamp(0.6rem, 1vw, 0.9rem)",fontWeight:600,color:"var(--p700)",whiteSpace:"nowrap",boxShadow:"0 4px 20px rgba(107,53,200,0.15)"}}>ZED Master Trainer</div>
              <div className="badge-float-2" style={{position:"absolute",zIndex:3,bottom:"22%",left:-16,background:"rgba(255,255,255,0.90)",backdropFilter:"blur(16px)",border:"1px solid rgba(255,255,255,0.95)",borderRadius:100,padding:"0.46rem 0.9rem",fontSize:"clamp(0.6rem, 1vw, 0.9rem)",fontWeight:600,color:"var(--p600)",whiteSpace:"nowrap",boxShadow:"0 4px 20px rgba(107,53,200,0.15)"}}>Dhyan Master</div>
              <div className="badge-float-3" style={{position:"absolute",zIndex:3,bottom:"6%",right:8,background:"rgba(255,255,255,0.90)",backdropFilter:"blur(16px)",border:"1px solid rgba(255,255,255,0.95)",borderRadius:100,padding:"0.46rem 0.9rem",fontSize:"clamp(0.6rem, 1vw, 0.9rem)",fontWeight:600,color:"var(--p800)",whiteSpace:"nowrap",boxShadow:"0 4px 20px rgba(107,53,200,0.15)"}}>30+ Years</div>
            </div>
            <div style={{textAlign:"center",marginBottom:"1.8rem"}}>
              <h3 className="font-display" style={{fontSize:"clamp(1.4rem, 3vw, 2rem)",fontWeight:700,color:"var(--ink)",letterSpacing:"-0.02em",marginBottom:"0.18rem"}}>Mishrilal Suriya</h3>
          ...
          <div className="reveal" style={{display:"flex",flexDirection:"column",gap:"1.6rem",paddingTop:"0.5rem",transitionDelay:"0.15s"}}>
            <div>
              ...
              <div style={{width:2.25rem,height:0.15625rem,background:"linear-gradient(90deg,var(--p500),var(--rose))",borderRadius:2,margin:"0.85rem 0"}}/>
      ...
      <div style={{maxWidth:1160,margin:"0 auto",padding:"0 2.5rem",position:"relative",zIndex:1}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"3rem",alignItems:"end",marginBottom:"3.5rem"}}>
          <div className="reveal">
            ...
            <div style={{width:2.25rem,height:0.15625rem,background:"linear-gradient(90deg,var(--p300),var(--rose))",borderRadius:2,margin:"0.85rem 0"}}/>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"1.4rem"}}>
          {cards.map(({n,en,hi,desc,glow,bar,delay})=>(
            <div key={n} className="fw-card reveal" style={{transitionDelay:`${delay}s`}}>
              ...
              <span className="font-display" style={{fontSize:"clamp(1.4rem, 3vw, 2rem)",fontWeight:700,color:"#fff",display:"block",lineHeight:1,marginBottom:"0.2rem"}}>{en}</span>
    ...
    <section id="philosophy" style={{padding:"7rem 0",position:"relative",zIndex:1}}>
      <div style={{maxWidth:1160,margin:"0 auto",padding:"0 2.5rem"}}>
        <div style={{textAlign:"center",marginBottom:"4rem"}} className="reveal">
          ...
          <div style={{width:2.25rem,height:0.15625rem,background:"linear-gradient(90deg,var(--p500),var(--rose))",borderRadius:2,margin:"0.85rem auto"}}/>
        ...
        <div className="reveal" style={{marginTop:"3.3rem",padding:"1.8rem 2rem",borderRadius:20,position:"relative",overflow:"hidden",background:"linear-gradient(145deg,var(--p900)0%,#20084a 50%,var(--p900)100%)",border:"1px solid rgba(255,255,255,0.14)",boxShadow:"0 14px 42px rgba(24,10,48,0.35)",transitionDelay:"0.28s"}}>
          ...
          <div style={{marginBottom:"1.35rem",position:"relative",zIndex:1}}>
            ...
            <div style={{width:2.25rem,height:0.15625rem,background:"linear-gradient(90deg,rgba(196,181,253,0.8),rgba(240,192,64,0.85))",borderRadius:2,margin:"0.85rem 0"}}/>
          </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:"2rem"}}>
          {cards.map(({n,title,body,delay})=>(
            <div key={n} className="svc-card reveal" style={{transitionDelay:`${delay}s`}}>
              ...
              <h3 className="font-display" style={{fontSize:"clamp(1.1rem, 2vw, 1.5rem)",fontWeight:700,color:"var(--gold)",marginBottom:"1rem",position:"relative",zIndex:1}}>{title}</h3>
          ...
          <div id="courses-card" className="svc-card reveal" style={{transitionDelay:"0.48s",gridColumn:"1 / -1"}}>
            ...
            <h3 className="font-display" style={{fontSize:"clamp(1.1rem, 2vw, 1.5rem)",fontWeight:700,color:"var(--gold)",marginBottom:"1rem",position:"relative",zIndex:1}}>Courses</h3>
    ...
    <section id="feedback" style={{padding:"0 0 6.5rem",position:"relative",zIndex:1}}>
      <div style={{maxWidth:1160,margin:"0 auto",padding:"0 2.5rem"}}>
        <div className="reveal glass" style={{padding:"2.2rem 2rem 2rem",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{width:2.25rem,height:0.15625rem,background:"linear-gradient(90deg,var(--p500),var(--rose))",borderRadius:2,margin:"0.85rem 0",position:"relative",zIndex:1}}/>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"5rem",alignItems:"center"}}>
          <div className="reveal">
            <span style={{fontSize:"0.62rem",fontWeight:600,letterSpacing:"0.3em",textTransform:"uppercase",color:"rgba(255,100,100,0.9)",display:"block",marginBottom:"0.6rem"}}>
              <span className="live-blink" style={{display:"inline-block",width:0.5rem,height:0.5rem,borderRadius:"50%",background:"#ff4444",marginRight:"0.5rem",boxShadow:"0 0 0.625rem rgba(255,68,68,0.9)"}}/>Daily Sessions on YouTube
            </span>
